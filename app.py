...
def extract_text_with_openai(image_bytes: bytes) -> tuple[str, str | None]:
    ...
    if not api_key:
        logger.warning("OpenAI OCR unavailable: missing OPENAI_API_KEY")
        return "", "OCR 服务未就绪：未配置 OpenAI API Key。"

    encoded = base64.b64encode(image_bytes).decode("utf-8")
    logger.info("Starting OpenAI OCR request (image_bytes=%s)", len(image_bytes))
    headers = {
    ...
    try:
        logger.info("Calling OpenAI Responses API for OCR")
        ...
        try:
            logger.warning("Responses API failed, falling back to Chat Completions")
            response = _post_openai_with_retries(
            ...
            if status == 429:
                logger.warning("OpenAI OCR rate limited (HTTP 429)")
                return (
                    "",
                    "OCR 服务请求过于频繁（HTTP 429）。请稍后再试或改用手动输入。",
                )
            if status:
                logger.error("OpenAI OCR request failed (HTTP %s)", status)
                return (
                    "",
                    f"OCR 服务请求失败（HTTP {status}），请稍后再试或改用手动输入。",
                )
            logger.exception("OpenAI OCR request failed with network error")
            return "", "OCR 服务请求失败，请稍后再试或改用手动输入。"
    if not content:
        logger.warning("OpenAI OCR returned empty text")
    ...
    *,
    ...
    for attempt in range(retries + 1):
        try:
            ...
            logger.info(
                "OpenAI OCR request attempt=%s status=%s url=%s",
                attempt + 1,
            ...
            status = getattr(exc.response, "status_code", None)
            logger.warning(
                "OpenAI OCR request failed attempt=%s status=%s url=%s",
                attempt + 1,
                status,
                url,
            )
...
def extract_text(image_bytes: bytes) -> tuple[str, str | None]:
    ...
    try:
        ...
        logger.warning("Local Tesseract OCR failed, falling back to OpenAI OCR")
        return extract_text_with_openai(image_bytes)

    if not text.strip():
        logger.warning("Local OCR returned empty text, falling back to OpenAI OCR")
